...
class Usuario {
    ...
    public function likesPost($postId) { // Checks if you like the Post
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'SELECT * FROM likedposts WHERE idUser = '.$this->id().' AND idPost = '.$postId.''; // Return the user ID
        $result = $conn->query($sql);
    ...
    public function likedAmount() {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'SELECT * FROM likedposts WHERE idUser = '.$this->id.''; // Return the user ID
        $result = $conn->query($sql);
        if ($result) {
    ...
    public function verifiedPost($id) { // Returns whether or not youve already verified a Pet
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'SELECT * FROM postvalidation WHERE idPost = '.$id.'';
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
        if ($row['userIdA'] == $this->id()
        || $row['userIdB'] == $this->id()
