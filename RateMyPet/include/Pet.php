...
class Pet {
    ...
    public static function insertar($petName, $petType, $petBreed, $petDescript, $treats, $owner_id) {
        ...
        if ($conn->query($query)) {
            ...
            // We add a validation row
            self::addValidationRow($petId);
    ...
    public function addPost($title, $tags, $description, $image) {
        ...
        $post->submitPost();
        $post->addValidationRow($post->idpost());
        return $post;
    }
    ...
    public static function getVerificationArray($idPet) {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'SELECT * FROM petvalidation WHERE petId = '.$idPet.''; // Return current ranking
        $rs = $conn->query($sql);
        $yes = 0;
        $no = 0;
        if ($pet_ = $rs->fetch_assoc()) {
            ...
            if ($pet_['userIdB'] != NULL) {
                $yes++; // 
            }
            if ($pet_['userIdC'] != NULL) { //
                $yes++; //
            }
    ...
    public static function verify($id) { // Verifies a Pet
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'UPDATE pets SET verified = 1 WHERE idPet = '.$id.''; // Verify
        $rs = $conn->query($sql);
        if ($rs) {
    ...
    public static function notVerify($id) { // Verifies a Pet
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'DELETE FROM pets WHERE idPet = '.$id.''; // Verify
        $rs = $conn->query($sql);
        if ($rs) {
    ...
    public static function getNotVerified() {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'SELECT * FROM pets WHERE verified = 0'; // Pets left to be verified
        $rs = $conn->query($sql);
        if($rs->num_rows > 0) {
            return $rs;
        } else return false;
    ...
    public static function sayYes($id) {
        ...
        if ($pet_ = $rs->fetch_assoc()) {
            if ($pet_['userIdA'] == NULL) {
                $sql = 'UPDATE petvalidation SET userIdA = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Accept
            } else if ($pet_['userIdB'] == NULL) {
                $sql = 'UPDATE petvalidation SET userIdB = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Accept
            } else if ($pet_['userIdC'] == NULL) { // This is the last Validation necessary
                $sql = 'UPDATE petvalidation SET userIdC = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Accept
            }
    ...
    public static function sayYesMod($id) {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'UPDATE petvalidation SET modId = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Return current ranking
        $rs = $conn->query($sql);
        if ($rs) {
            Pet::verify($id);
            return true;
    ...
    public static function sayNo($id) {
        ...
        if($pet_ = $rs->fetch_assoc()) {
            if ($pet_['notAId'] == NULL) {
                $sql = 'UPDATE petvalidation SET notAId = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Deny
            } else if ($pet_['notBId'] == NULL) {
                $sql = 'UPDATE petvalidation SET notBId = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Deny
            } else if ($pet_['notCId'] == NULL) { // Deny
                $sql = 'UPDATE petvalidation SET notCId = '.$_SESSION['user']->id().' WHERE petId = '.$id.''; // Deny
            }
            $rs = $conn->query($sql); // Execute
            if ($rs) return true;
            else return false;
    ...
    public static function addValidationRow($id) {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'INSERT INTO petvalidation VALUES ('.$id.', NULL, NULL, NULL, NULL, NULL, NULL, NULL)'; // Return current ranking
        $rs = $conn->query($sql);
        if ($rs) {
            return true;
        } else return false;
    ...
    public static function deleteValidationRow($id) {
        $app = Aplicacion::getSingleton();
        $conn = $app->conexionBd();
        $sql = 'DELETE FROM petvalidation WHERE petId = '.$id.''; // Delete The Row
        $rs = $conn->query($sql);
        if ($rs) {
            return true;
        } else return false;
