...
<body>
    ...
    <div class="content">
        ...
        if ($pets || $posts) {
            echo ' 
                        <h1>▪️ ◾ ◼️ ⬛ <a class="verify" href="verify.php">New Pets / Posts need to be verified!</a> ⬛ ️◼ ◾ ️▪</h1>
                    ';
        }
        ?>
        <hr>
        ...
        if (!$postList) {
            ...
            while ($row = $postList->fetch_assoc()) {
                ...
                if ($_SESSION['user']->likesPost($post->idPost())) {
                    echo '<a id="like" href="" onClick="likePost(\'' . $post->idPost() . '\', \'unlike\');">
                        <p class="iright"><i class="fa fa-heart like"></i> ' . $post->likes() . ' </p>
                    </a>';
                } else {
                    echo '<a id="like" href="" onClick="likePost(\'' . $post->idPost() . '\', \'like\');">
                        <p class="iright"><i class="fa fa-heart like"></i> ' . $post->likes() . ' </p>
                    </a>';
                }
                echo '</div>';
    ...
    <script src="js/slickSettingsFeed.js"></script>
